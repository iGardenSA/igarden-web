...
|---|---|---|
| **#41** | 🔴 **OPEN — لم يُغلق** | **مادّته طُويت في هذا الفرع** (‏sitemap + `robots` allow `/api/og`) ◐؛ الـPR نفسه ما زال مفتوحاً ويحتاج إغلاقاً صريحاً |
...
⚠ **لا تُسجَّل #41 «مغلقة» قبل إغلاقها فعلاً** — الحالة أعلاه من `gh pr view` مباشرةً.
...
- **`docs/project-state-reconcile-aug`** @ `bcbc4ad` — نقل `PROJECT-STATE.md` إلى الجذر + قرار إسناد الليد + سجلّ موجة الدمج. **لم يُدفع ولم يُدمج.**
  ⚠ هذا الملفّ (في فرع `wave1a-nav`) يكرّر النقل نفسه — **يلزم حسم أيّهما المصدر قبل دمج أيٍّ منهما.**
