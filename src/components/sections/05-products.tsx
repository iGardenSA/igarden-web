      ...
      <div className="container-igarden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-5 md:gap-6 auto-rows-fr">
          {products.map((product, i) => {
            ...
            return (
              <FadeIn key={product.id} delay={i * 80} className="lg:col-span-3">
                <Link href={`/products/${product.id}`} className="group relative bg-white rounded-2xl border border-[var(--color-border)] p-6 md:p-7 shadow-[var(--shadow-soft)] hover:shadow-[var(--shadow-md)] hover:-translate-y-1 transition-all duration-300 flex flex-col h-full">
